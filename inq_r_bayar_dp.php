...
(select a.nama_cpw, a.created, a.catatan, ifnull(sum(b.nilai),'-')bayar
from client a
left join client_bayar b on a.id_client=b.id_client
where 1=1 and a.status='Aktif' $filterSQL 
group by a.id_client
order by bayar desc, created asc) b ";
...
$runSQL = "	select a.nama_cpw, a.created, a.catatan, ifnull(sum(b.nilai),'-')bayar
from client a
left join client_bayar b on a.id_client=b.id_client
where 1=1 and a.status='Aktif' $filterSQL
group by a.id_client
order by bayar desc, created asc LIMIT $offsetRecord, $listRecord  ";
...
<tr bgcolor='#A7A7A7' height="25" align="center">
	<td width="63" >No</td>
	<td width="142" >Nama CPW</td>
    <td width="143" >Tanggal& Waktu <br /> Kedatangan</td>
	<td width="349" >Catatan</td>
	<td width="128" >Bayar</td>
...
?>
	<td><?= $rec['nama_cpw'];?></td>
	<td align="center"><?= $rec['created'];?>/<?= $rec['nama_cpp'];?></td>
	<td><?= $rec['catatan'];?></td>
	<td align="center"><?= $rec['bayar'];?></td>

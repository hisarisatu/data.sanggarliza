...
?>
<tr><td>Total Biaya (- diskon)</td><td colspan=2>: Rp. <?=number_format($total-$diskon+$ppn,0)?></td></tr>
<?  
} else {
?>
...
<tr><td colspan=2 align=center><a href="<?="?menu=$menu&uid=$uid&page=payment";?>" > 
...
$sql="insert into client_bayar values ('$id_client',null,'$tgl_bayar','$nilai','$keterangan','$pembayar','$catatan','$SAH[id_user]', '$REMOTE_ADDR','$sekarang','$no_urut','$no_kw')";
//echo "<br>$sql";
mysql_query($sql);
$act=null;
}
...
<td><?=mysql_result($rk,$k,"id_client")?></td>
<td><?=mysql_result($rk,$k,"created")?></td>
<td align=center>
<? $id_client=mysql_result($rk,$k,"id_client");
	$id_bayar=mysql_result($rk,$k,"id_bayar");
	echo "<a href=\"javascript:void(window.open('cetak_bukti.php?id_client=$id_client&id_bayar=$id_bayar&id_user=$SAH[id_user]&login_ip=$REMOTE_ADDR&created=$sekarang&no_kw=".mysql_result($rk,$k,"no_kwitansi")."&printed=$sekarang '))\">"; ?><img border=0 src='images/Printer.png'></a></td>
<td align=center>
<? $id_client=mysql_result($rk,$k,"id_client");
	$id_bayar=mysql_result($rk,$k,"id_bayar");
	echo "<a href=?menu=$menu&uid=$uid&page=client_nota_edit&id_client=$id_client&id_bayar=$id_bayar&tanggal=$tgl_bayar&tno_urut=$no_urut&no_kw=$no_kw>"?><img border=0 src='images/edit.gif'></a></td>

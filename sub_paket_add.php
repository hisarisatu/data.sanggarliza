...
if ((strlen($run) < 1) and ($id <> "")){ 
	$runSQL = "select id_sublayanan, id_layanan, harga_dasar, detail_layanan, int_ext, date_format(tgl_habis,'%m/%d/%Y') expire from p_sublayanan where id_sublayanan='$id'";
        //echo $runSQL;
...
if (strlen($run) > 1){ 
	...
	if (($ok == 1) and ($id == "")){
		$registerInvalid = 1;
		$runSQL = "insert into p_sublayanan(id_layanan,detail_layanan,harga_dasar,int_ext,tgl_habis) VALUES ('$id_layanan','$detail_layanan', '$harga_paket','$sumberdaya','$expire')";
		//echo $runSQL;
		$insert = mysql_query($runSQL, $connDB);
		$id = mysql_insert_id($connDB);
	} else if (($ok == 1) and ($id <> "")){
		$registerInvalid = 1;
		$runSQL = "update p_sublayanan set id_layanan='$id_layanan',harga_dasar='$harga_paket', detail_layanan='$detail_layanan',int_ext='$sumberdaya',tgl_habis='$expire' where id_sublayanan='$id'";
		//echo $runSQL;
   ...
   <td width="100%" align="center" vAlign="top">
	 ...
	 <table border="0" cellpadding="5" cellspacing="0" width="650">
        <tr>
            ...
            <td width="65%"><select name="id_produk" onchange="javascript:this.form.submit();">
<?
...
$sql="select id_layanan,layanan from p_layanan where id_produk='$id_produk'";
$rs=mysql_query($sql);
for($a=0;$a<mysql_num_rows($rs);$a++){
echo "<option ";
...
echo "</option>";
        ...
        <tr>
            <td width="35%" align="right">Sumberdaya</td>
            <td width="65%"><select name="sumberdaya">
<option <? if($sumberdaya=="internal")echo" selected ";?> value="internal">Internal
<option <? if($sumberdaya=="external")echo" selected ";?> value="external">External
</select><font color="#FF0000"><b>*</b></font> <?=$isumberdaya;?></td>
        </tr>
<tr><td colspan="2" width="100%" align="center"> <br><b>Expire Date</b> </td></tr>
        <tr>
                     <td width="35%" align="right"> Tanggal </td><td> <input type='text' name='expire' size='11' value='<?=$expire?>'>
...
//registerInvalid
	...
	$runSQL = "select id_sublayanan, id_layanan, harga_dasar, detail_layanan, int_ext from p_sublayanan where id_sublayanan='$id'";
	$result = mysql_query($runSQL, $connDB);
	if ($row = mysql_fetch_array ($result)) {
		$id_sublayanan = $row[id_sublayanan];
		$id_layanan = $row[id_layanan];
...
<tr>
       ...
       <tr>
            <td width="35%" align="right">Sumberdaya: </td>
            <td width="65%"><font class="datafield"><?=$sumberdaya;?></font></td>
        </tr>
	 </table>

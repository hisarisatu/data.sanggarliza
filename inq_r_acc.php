...
$pageFilter ="&tanggal=$tanggal&tanggal2=$tanggal2&id_tipe_acc=$id_tipe_acc&id_acc=$id_acc&id_tipe_kain=$id_tipe_kain&id_kain=$id_kain";

				//if (!$tanggal) {$tanggal=date_format(current_date(),'%Y-%m-1');}
				//if (!$tanggal2) {$tanggal2=date_format(current_date(),'%Y-%m-%d');}
                if ($id_tipe_acc==0) {$id_tipe_acc='%';}
				if ($id_acc==0) {$id_acc='%';}


                if($id_tipe_acc !="") { $filterSQL = " and g.id_tipe_acc like '$id_tipe_acc' "; }
				if($id_acc !="") { $filterSQL .= " and g.id_acc like '$id_acc' "; }
...
and g.id_tipe_acc=h.id_tipe_acc
and g.id_tipe_acc=i.id_tipe_acc
and g.id_acc=i.id_layanan
$filterSQL) b ";
...
c.nama_cpw,c.nama_cpp,e.detail_layanan,h.keterangan tipe_acc,i.layanan jenis_acc,g.jml_acc,i.qty kap_acc
from acara a,p_acara b,client c,paket_sub_paket d,p_sublayanan e, 
client_busana g,p_acc_tipe h,p_acc i
where a.id_acara=b.id_acara 
and a.id_client=c.id_client
and a.id_paket=d.id_paket    
and a.id_acara=g.id_acara 
...
<tr>
<td><b>Tipe Accecoris </b></td><td><b>:</b></td>
<td>
<?
js_submit();
$sqltipebaju="select distinct id_tipe_acc,keterangan from p_acc_tipe where upper(keterangan) not like '%KAIN%' union select '0','--Pilih Tipe Accesoris--' from dual";
generate_select_event("id_tipe_acc",$sqltipebaju,$id_tipe_acc,"submit_form()"); 
?>
</td>
</tr>
<tr>
<td><b>Jenis Accecoris </b></td><td><b>:</b></td>
<td>
<?
js_submit();
$sqlbaju="select * from (select distinct id_layanan,layanan from p_acc where id_tipe_acc like '$id_tipe_acc' union select '0','--Pilih Jenis Accecoris--' from dual) a order by layanan";
generate_select("id_acc",$sqlbaju,$id_acc);
?>
</td>
</tr>
...
<tr bgcolor='#A7A7A7' height="25" align="center">
	...
	<td rowspan='2'>Jenis Accecoris</td>
	<td colspan='3'>Jumlah Accecoris</td>
</tr>
<tr bgcolor='#A7A7A7' height="25" align="center">
	<td>Kap</td>
	<td>Pakai</td>
...
?>
	...
	<td><?= $rec['tipe_acc'];?>/<?= $rec['jenis_acc'];?></td>
	<td><?= $rec['kap_acc']; ?></td>
	<td><?= $rec['jml_acc']; ?></td>
	<td><?= $rec['kap_acc']-$rec['jml_acc']; ?></td>

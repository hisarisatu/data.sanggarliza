...
// 09 oktober 2010, lastupdate 28 oktober 2015 @restatimur
...
if ((strlen($run) < 1) and ($id <> "")){ 
	$runSQL = "select id_layanan, id_tipe_baju, layanan,qty,warna, daerah from p_baju where id_layanan='$id'";
        #echo $runSQL;
...
if (strlen($run) > 1){ 
	...
	if (($ok == 1) and ($id == "")){
		$registerInvalid = 1;
		$runSQL = "insert into p_baju(id_tipe_baju,layanan,qty,daerah,warna) VALUES ('$id_tipe_baju','$layanan','$qty','$daerah', '$id_warna')";
		echo $runSQL;
		$insert = mysql_query($runSQL, $connDB);
		$id = mysql_insert_id($connDB);
	} else if (($ok == 1) and ($id <> "")){
		$registerInvalid = 1;
		$runSQL = "update p_baju set id_tipe_baju='$id_tipe_baju',layanan='$layanan',qty='$qty',daerah='$daerah', warna='$id_warna' where id_layanan='$id'";
		//echo $runSQL;
...
//registerInvalid
	...
	$runSQL = "select id_layanan, id_tipe_baju,layanan,qty, warna from p_baju where id_layanan='$id'";
	$result = mysql_query($runSQL, $connDB);
	if ($row = mysql_fetch_array ($result)) {
		$id_layanan = $row[id_layanan];
		$id_tipe_baju = $row[id_tipe_baju];

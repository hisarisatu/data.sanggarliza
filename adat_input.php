...
if (strlen($run) > 1){ 
	unset($del_acara);
	$runSQL = "select id_adat, tarif from p_adat order by id_adat asc";
	$result = mysql_query($runSQL, $connDB);
	while ($row = mysql_fetch_array ($result)) {
		$vrb_adat = "adat".$row[id_adat];
		$vlu_adat = $$vrb_adat;
		$vrb_catatan = "catatan".$row[id_adat];
		$vlu_catatan = $$vrb_catatan;
		$vrb_harga = "harga".$row[id_adat];
		$vlu_harga = $$vrb_harga;
		if ($vlu_harga <= 0){ $vlu_harga=$row[tarif]; }

		if ($vlu_adat > 0){
			$runSQL2 = "select id_client from order_adat where id_client='$id' and id_acara='$id_acara' and id_adat='$row[id_adat]'";
			$result2 = mysql_query($runSQL2, $connDB);
			if ($row2 = mysql_fetch_array($result2)) {
				//update data
				$runSQL2 = "update order_adat set catatan='$vlu_catatan', harga_baru=$vlu_harga, user_update='$SAH[id_user]', ip_update='$REMOTE_ADDR', last_update=now() where id_client='$id' and id_acara='$id_acara' and id_adat='$row[id_adat]'";
				$update2 = mysql_query($runSQL2, $connDB);
			}else{
				//insert data
				$runSQL2 = "insert into order_adat (id_client, id_acara, id_adat, catatan, harga_baru, id_user, login_ip, created) VALUES ('$id', '$id_acara', '$row[id_adat]', '$vlu_catatan', $vlu_harga, '$SAH[id_user]', '$REMOTE_ADDR', now())";
		...
		if ($vlu_adat <= 0){
			$runSQL2 = "delete from order_adat where id_client='$id' and id_acara='$id_acara' and id_adat='$row[id_adat]'";
...
if ($registerInvalid <> 1){
	...
	unset($ccc);
	$runSQL = "select id_adat, nama, keterangan, foto, tarif from p_adat order by id_adat asc";
	$result = mysql_query($runSQL, $connDB);
	while ($row = mysql_fetch_array ($result)) {
		$vrb_adat = "adat".$row[id_adat];
		$vlu_adat = $$vrb_adat;
		...
		if ($vlu_adat == $row[id_adat]){ $checked="checked"; }else{ unset($checked); };

		$ccc++;
		if ($ccc%2 > 0){ $color="#EBEFFA"; }else{ $color="#D7E0F4"; };
		$htmlData .= "
			<tr bgcolor='$color' onmouseover='bgColor=\"#FDD0D8\"' onmouseout='bgColor=\"$color\"'>
				<td align='center'> <input type='checkbox' $checked name='$vrb_adat' value='$row[id_adat]'> </td>
				<td>
...
?>
   ...
   <form method="POST" name="form" action="<?="?menu=$menu&uid=$uid&page=$page";?>">
	 <font class="titledata"><b>Input/Edit Upacara Adat</b><br></font>
	 <table width="100%" border="0" cellpadding="0" cellspacing="0">
	  <tr>
			 ...
			 <table width='600' cellspacing='1' cellpadding='3'>
			  <?=$htmlData;?>
				...
				<tr>
					...
					Keterangan : <br>
					- Untuk memilih Upacara Adat berilah tanda Cek pada kolom sebelah kiri Nama Upacara Adat.<br>

...
if ((strlen($run) < 1) and ($id <> "")){ 
	$runSQL = "select id_layanan, id_tipe_sepatu, tipe_ukuran_sepatu,jumlah,id_ukuran_sepatu,jumlah from p_ukuran_sepatu where id_ukuran_sepatu='$id'";
        #echo $runSQL;
	$result = mysql_query($runSQL, $connDB);
	if ($row = mysql_fetch_array ($result)) {
		
		$id_layanan = $row[id_layanan];
		$id_tipe_baju = $row[id_tipe_sepatu];
		$id_ukuran_baju = $row[tipe_ukuran_sepatu];
		$jumlah = $row[jumlah];
        $id=$row[id_ukuran_sepatu];    
	};//if
};//if-id
...
if (strlen($run) > 1){ 
	...
	if (($ok == 1) and ($id == "")){
		$registerInvalid = 1;
		$runSQL = "insert into p_ukuran_sepatu (id_layanan,id_tipe_sepatu,tipe_ukuran_sepatu,jumlah) VALUES ('$id_layanan','$id_tipe_baju','$id_ukuran_baju','$jumlah')";
		#echo $runSQL;
		$insert = mysql_query($runSQL, $connDB);
		$id = mysql_insert_id($connDB);
	} else if (($ok == 1) and ($id <> "")){
		$registerInvalid = 1;
		$runSQL = "update p_ukuran_baju set id_tipe_baju='$id_tipe_baju',tipe_ukuran_sepatu='$id_ukuran_sepatu',jumlah='$jumlah' where id_ukuran_baju='$id'";
		#echo $runSQL;
  ...
  <tr>
   <td width="100%" align="center" vAlign="top">
     ...
     <td valign="bottom" align="right">
			 ...
			 <a href="<?="?menu=$menu&uid=$uid&page=p_sepatu_ukuran";?>"><b>Tambah Ukuran Sepatu</b></a>
			 <img src="images/arrow2.gif" border="0">
			 <a href="<?="?menu=$menu&uid=$uid&page=p_sepatu";?>"><b>List sepatu</b></a>
              <img src="images/arrow2.gif" border="0">
			 <a href="<?="?menu=$menu&uid=$uid&page=p_list_ukuran_sepatu";?>"><b>List Ukuran Sepatu</b></a>
		 </td>
	  </tr>
...
echo "</option>";
	...
	<tr>
            ...
            <td width="65%"><?
$sqlpetugas="select distinct id_layanan, layanan from p_sepatu
union select 0,'--Pilih Sepatu--' from dual";
generate_select("id_layanan",$sqlpetugas,$id_layanan); 
?><font color="#FF0000"><b>*</b></font> <?=$ilayanan;?></td>
        </tr>
        
        <tr>
			<td width="35%" align="right">Tipe Ukuran Sepatu</td>
            <td width="65%"><?
$sqlp="select distinct id_ukuran_baju, nama_ukuran_baju from p_jenis_ukuran_baju
			 union select 0,'--Pilih Ukuran Sepatu--' from dual";
...
//registerInvalid
	...
	$runSQL = "select id_layanan, id_tipe_sepatu, tipe_ukuran_sepatu, jumlah, id_ukuran_sepatu from p_ukuran_sepatu where id_ukuran_sepatu='$id' ";
	$result = mysql_query($runSQL, $connDB);
	if ($row = mysql_fetch_array ($result)) {
...
<table border="0" width="850" cellspacing="0" cellpadding="0" align="center">
  ...
  <tr>
   <td width="100%" align="center" vAlign="top">
	 ...
	 <p>&nbsp;</p>
	 <img src="images/arrow2.gif" border="0">
	 <a href="<?="?menu=$menu&uid=$uid&page=p_sepatu_ukuran";?>"><b>Input Ukuran Baju Lagi</b></a>
   </td>
  </tr>
  </form>
</table>

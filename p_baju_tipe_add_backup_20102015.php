...
if ((strlen($run) < 1) and ($id <> "")){ 
	$runSQL = "select id_tipe_baju, keterangan from p_baju_tipe where id_tipe_baju='$id'";
        //echo $runSQL;
...
if (strlen($run) > 1){ 
	...
	if (($ok == 1) and ($id == "")){
		$registerInvalid = 1;
		$runSQL = "insert into p_baju_tipe VALUES (null,'$keterangan')";
		//echo $runSQL;
		$insert = mysql_query($runSQL, $connDB);
		$id = mysql_insert_id($connDB);
	} else if (($ok == 1) and ($id <> "")){
		$registerInvalid = 1;
		$runSQL = "update p_baju_tipe set keterangan='$keterangan' where id_tipe_baju='$id'";
		//echo $runSQL;

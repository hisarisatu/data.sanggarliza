...
if (strlen($run) > 1){ 
	...
	if (($ok == 1) and ($id == "")){
		$registerInvalid = 1;
		$runSQL = "insert into paket (nama_paket, harga_paket, uraian_paket, tgl_inisial, tgl_expire, id_user, login_ip, created) VALUES ('$nama_paket', '$harga_paket', '$uraian_paket', '$tgl_inisial', '$tgl_expire', '$SAH[id_user]', '$REMOTE_ADDR', now())";
		//echo $runSQL;
		$insert = mysql_query($runSQL, $connDB);
		$id = mysql_insert_id($connDB);
                $runSQL = "update paket_sub_paket set id_paket='$id' where id_paket=0";
                $u = mysql_query($runSQL);
	} else if (($ok == 1) and ($id <> "")){
		$registerInvalid = 1;
		$runSQL = "update paket set nama_paket='$nama_paket', harga_paket='$harga_paket', uraian_paket='$uraian_paket', tgl_inisial='$tgl_inisial', tgl_expire='$tgl_expire', user_update='$SAH[id_user]', ip_update='$REMOTE_ADDR', last_update=now() where id_paket='$id'";
		//echo $runSQL;
   ...
   <td width="100%" align="center" vAlign="top">
	 ...
	 <table border="0" cellpadding="5" cellspacing="0" width="650">
		...
		<tr>
			...
			<td width="65%"><input type="text" name="harga_paket" size="15" value="<?=htmlentities(stripslashes($harga_paket));?>"><font color="#FF0000"></font> <?=$iharga_paket;?></td>
		</tr>
		<tr valign=top>
			<td width="35%" align="right">Uraian Paket</td>
			<td width="65%"><iframe src="sub_paket_input.php?id=<?=$id?>" width="800" frameborder=0></iframe>
...
<table border="0" width="850" cellspacing="0" cellpadding="0" align="center">
  ...
  <tr>
   <td width="100%" align="center" vAlign="top">
	 ...
	 <table width="100%" border="0" cellpadding="0" cellspacing="0">
	  <tr><td colspan="2" align="right"><a href='<?="?menu=$menu&uid=$uid&page=paket_input&id=$id";?>'><img border='0' src='images/edit.gif' title='Edit Data'></a> &nbsp; &nbsp; </td></tr>
	  <tr><td colspan="2"> <hr size="1" color="#4B4B4B"> </td></tr>
	  <tr>
	 ...
	 <table border="0" cellpadding="5" cellspacing="0" width="650">
		...
		<tr>
			<td width="35%" align="right">Harga Paket : </td>
			<td width="65%"><font class="datafield"><?=number_format($harga_paket,0);?></font></td>
		</tr>
		<tr valign=top>
			<td width="35%" align="right">Uraian Paket : </td>
			<td width="65%"><font class="datafield">
<?
$sql="select a.detail_layanan,b.jml_orang from p_sublayanan a, paket_sub_paket b where a.id_sublayanan=b.id_sublayanan and b.id_paket='$id'";
$rs=mysql_query($sql);

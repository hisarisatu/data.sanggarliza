...
if ((strlen($run) < 1) and ($id <> "")){ 
	$runSQL = "select id_butik, nama, produk, tgl_acara, tgl_dateline, fitting, hp, keterangan,harga, acara from butik where id_butik='$id'";
...
//if (strlen($id_pegawai) < 1){ $iid_pegawai = "<br><font color='#FF0000' size='1'><i>* Petugas CS tidak valid"; $ok=0; }
	...
	if (($ok == 1) and ($id == "")){
		$registerInvalid = 1;
			$runSQL = "insert into butik (nama, produk, tgl_acara, tgl_dateline, acara, hp, keterangan, created, harga, fitting) VALUES ('$nama', '$produk', '$new_tanggal1','$new_tanggal','$acara','$hp','$keterangan', now(), '$harga', '$fitting')";
		//echo $runSQL;
		$insert = mysql_query($runSQL, $connDB);
		$id = mysql_insert_id($connDB);
	} else if (($ok == 1) and ($id <> "")){
		$registerInvalid = 1;
		$runSQL = "update butik set nama='$nama', produk='$produk', tgl_acara='$new_tanggal1',tgl_dateline='$new_tanggal', acara='$acara', hp='$hp', keterangan='$keterangan', harga='$harga', fitting='$fitting' where id_butik='$id'";
		//echo $runSQL;
  ...
  <tr>
   <td width="100%" align="center" vAlign="top">
     ...
     <td valign="bottom" align="right">
			 ...
			 <a href="<?="?menu=$menu&uid=$uid&page=butikt";?>"><b>Back Data Client</b></a>
		 </td>
	  </tr>
	 ...
	 <table border="0" cellpadding="5" cellspacing="0" width="650">
        ...
        <tr>
			<td width="100%" align="right">Tanggal Acara</td><td> <input type='text' name='tanggal1' size='11' value='<?=$new_tanggal?>'>
		...
		<tr>
         ...
         <tr>
			...
			<td width="65%"><input type="text" name="harga" size="15" value="<?=$harga;?>">			  <?=$iharga;?></td>
				...
				<tr>
			<td width="100%" align="right">Deadline</td><td> <input type='text' name='tanggal' size='11' value='<?=$new_tanggal1?>'>
		...
		<tr>
        	...
        	<tr>
			<td width="100%" align="right">Fitting</td>
			<td width="65%"><textarea name="fitting" cols="50" rows="5" ><?=$fitting;?></textarea> <font color="#FF0000"><b>*</b></font> <?=$idetail;?></td>
		</tr>
...
//registerInvalid
	...
	$runSQL = "select id_butik, nama, produk, tgl_acara, tgl_dateline, acara, hp, keterangan,harga,fitting from butik where id_butik='$id'";
	$result = mysql_query($runSQL, $connDB);
	if ($row = mysql_fetch_array ($result)) {
	$id_butik = $row[id_butik];
		$nama = $row[nama];
		 ...
		 <table border="0" cellpadding="5" cellspacing="0" width="100%">
			...
			<tr>
			  ...
			  <td ><font class="datafield"><?=$keterangan;?></font></td>
                 ...
                 <tr>
			  <td  align="right">Fitting :</td>
			  <td ><font class="datafield"><?=$fitting;?></font></td>
			  </tr>
		 </table>

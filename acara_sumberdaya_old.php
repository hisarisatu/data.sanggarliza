...
for($l=0;$l<@mysql_num_rows($res);$l++){
	...
	<table cellpadding=4 cellspacing=0 border=1>
	<?php
		$sql="select distinct a.id_sublayanan,a.detail_layanan,b.jml_orang,'0' id_paket from p_sublayanan a, pesanan_plus b 
		where a.id_sublayanan=b.id_sublayanan and b.id_client='$id_client' and b.id_acara='$id_acara' 
		and a.id_layanan='$id_layanan' 
		union select distinct a.id_sublayanan,a.detail_layanan,b.jml_orang,b.id_paket from p_sublayanan a, paket_sub_paket b, acara c 
		where a.id_sublayanan=b.id_sublayanan and b.id_paket=c.id_paket and c.id_acara='$id_acara' and c.id_client='$id_client' 
		and a.id_layanan='$id_layanan'";
		#echo $sql;
                #echo "<pre>".print_r($sql,true)."</pre>";
		$rs2=mysql_query($sql);
		...
		<tr>
			...
			switch($id_layanan)
			{
				case 2:
					?>
					<td><?="<a href=\"javascript:void(window.open('assign_baju.php?jml_baju=$jml_vol&id_client=$id_client&id_acara=$id_acara&id_paket=$id_paket&id_plus=$id_sublayanan','operator','toolbar=0,width=500,height=500,top=0, left=60'));\">"?>
						<?php
					...
					<td>
					<?="<a href=\"javascript:void(window.open('assign_kain.php?jml_kain=$jml_vol&id_client=$id_client&id_acara=$id_acara&id_paket=$id_paket&id_plus=$id_sublayanan','operator','toolbar=0,width=500,height=500,top=0, left=60'));\">"?>
					<?php $sbl2=mysql_query("select count(*) jumlah from client_busana where id_acara='$id_acara' and id_client='$id_client' and id_plus='$id_sublayanan' and id_paket='$id_paket' and id_kain!=0 ");
						$cl2=mysql_result($sbl2,0,"jumlah");
						if($cl2!=0){ 
							echo "<img src='images/ok.png' border=0 width=12 height=12>";
						}else{?>
							<img src='images/arw.gif' border=0 title='Pilih Kain'>
						<? } ?>
						</a></td>
				<?php break;
			}
			...
			{
				case 12:
				if (($id_sublayanan==1350) or ($id_sublayanan==1351))
				{
					?>
					<td><?="<a href=\"javascript:void(window.open('assign_sepatu.php?jml_sepatu=$jml_vol&id_client=$id_client&id_acara=$id_acara&id_paket=$id_paket&id_plus=$id_sublayanan','operator','toolbar=0,width=500,height=200,top=0, left=60'));\">"?>
						<?php
						$sbl=mysql_query("select count(*) jumlah from client_busana where id_acara='$id_acara' and id_client='$id_client' and id_plus='$id_sublayanan' and id_paket='$id_paket' ");
						$cl=mysql_result($sbl,0,"jumlah");
						if($cl!=0){ 
							...
							<img src='images/arw.gif' border=0 title='Pilih Accecoris'>
						<? } ?>
						</a></td>
				<? break;
				} else 
				{
					?>
					<td><?="<a href=\"javascript:void(window.open('assign_acc.php?jml_acc=$jml_vol&id_client=$id_client&id_acara=$id_acara&id_paket=$id_paket&id_plus=$id_sublayanan','operator','toolbar=0,width=500,height=500,top=0, left=60' ));\">"?>
						<?php
						$sbl=mysql_query("select count(*) jumlah from client_busana where id_acara='$id_acara' and id_client='$id_client' and id_plus='$id_sublayanan' and id_paket='$id_paket' ");
						$cl=mysql_result($sbl,0,"jumlah");
						if($cl!=0){ 

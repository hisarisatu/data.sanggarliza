...
if (strlen($run) > 1){ 
	unset($del_acara);
	$runSQL = "select id_mc, tarif from p_mc order by id_mc asc";
	$result = mysql_query($runSQL, $connDB);
	while ($row = mysql_fetch_array ($result)) {
		$vrb_mc = "mc".$row[id_mc];
		$vlu_mc = $$vrb_mc;
		$vrb_catatan = "catatan".$row[id_mc];
		$vlu_catatan = $$vrb_catatan;
		$vrb_harga = "harga".$row[id_mc];
		$vlu_harga = $$vrb_harga;
		if ($vlu_harga <= 0){ $vlu_harga=$row[tarif]; }

		if ($vlu_mc > 0){
			$runSQL2 = "select id_client from order_mc where id_client='$id' and id_acara='$id_acara' and id_mc='$row[id_mc]'";
			$result2 = mysql_query($runSQL2, $connDB);
			if ($row2 = mysql_fetch_array($result2)) {
				//update data
				$runSQL2 = "update order_mc set catatan='$vlu_catatan', harga_baru=$vlu_harga, user_update='$SAH[id_user]', ip_update='$REMOTE_ADDR', last_update=now() where id_client='$id' and id_acara='$id_acara' and id_mc='$row[id_mc]'";
				$update2 = mysql_query($runSQL2, $connDB);
			}else{
				//insert data
				$runSQL2 = "insert into order_mc (id_client, id_acara, id_mc, catatan, harga_baru, id_user, login_ip, created) VALUES ('$id', '$id_acara', '$row[id_mc]', '$vlu_catatan', $vlu_harga, '$SAH[id_user]', '$REMOTE_ADDR', now())";
		...
		if ($vlu_mc <= 0){
			$runSQL2 = "delete from order_mc where id_client='$id' and id_acara='$id_acara' and id_mc='$row[id_mc]'";
...
if ($registerInvalid <> 1){
	...
	unset($ccc);
	$runSQL = "select id_mc, nama, keterangan, foto, tarif from p_mc order by id_mc asc";
	$result = mysql_query($runSQL, $connDB);
	while ($row = mysql_fetch_array ($result)) {
		$vrb_mc = "mc".$row[id_mc];
		$vlu_mc = $$vrb_mc;
		...
		if ($vlu_mc == $row[id_mc]){ $checked="checked"; }else{ unset($checked); };

		$ccc++;
		if ($ccc%2 > 0){ $color="#EBEFFA"; }else{ $color="#D7E0F4"; };
		$htmlData .= "
			<tr bgcolor='$color' onmouseover='bgColor=\"#FDD0D8\"' onmouseout='bgColor=\"$color\"'>
				<td align='center'> <input type='checkbox' $checked name='$vrb_mc' value='$row[id_mc]'> </td>
				<td>
...
?>
   ...
   <form method="POST" name="form" action="<?="?menu=$menu&uid=$uid&page=$page";?>">
	 <font class="titledata"><b>Input/Edit Master of Ceremony (MC)</b><br></font>
	 <table width="100%" border="0" cellpadding="0" cellspacing="0">
	  <tr>

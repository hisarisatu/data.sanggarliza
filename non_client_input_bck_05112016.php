...
if ((strlen($run) < 1) and ($id <> "")){ 
	$runSQL = "select id_non_client, nama, divisi, detail, jumlah, tanggal, jenis_bayar, bank from non_client where id_non_client='$id'";
...
//if (strlen($id_pegawai) < 1){ $iid_pegawai = "<br><font color='#FF0000' size='1'><i>* Petugas CS tidak valid"; $ok=0; }
	...
	if (($ok == 1) and ($id == "")){
		...
		$runSQL = "insert into non_client (nama, divisi, detail, jumlah, tanggal, jenis_bayar, bank, created, kwitansi, id_user, ip_login) VALUES ('$nama', '$id_jenis_bagian', '$detail', '$jumlah', '$new_tanggal', '$id_jenis_bayar', '$id_bank', now(),'$newID','$SAH[id_user]', '$REMOTE_ADDR')";
		//echo $runSQL;
		$insert = mysql_query($runSQL, $connDB);
		$id = mysql_insert_id($connDB);
	} else if (($ok == 1) and ($id <> "")){
		$registerInvalid = 1;
		$runSQL = "update non_client set nama='$nama', divisi='$id_jenis_bagian', detail='$detail', jumlah='$jumlah', tanggal='$new_tanggal', jenis_bayar='$id_jenis_bayar', bank='$id_bank', id_user='$SAH[id_user]', created=now(), ip_login='$REMOTE_ADDR' where id_non_client='$id'";
		//echo $runSQL;
   ...
   <td width="100%" align="center" vAlign="top">
     ...
     <td width="50%" valign="top" align="center">
		 <table border="0" cellpadding="5" cellspacing="0" width="100%">
            ...
            <tr>
                <td  align="right">Jenis Bayar :</td>
                <td ><font class="datafield"><?=$jenis_bayar;?></font></td>
            </tr>
			<tr>
				<td  align="right">Bank :</td>
				<td ><font class="datafield"><?=$bank;?></font></td>
			</tr>
		 </table>

...
from 
(
select sum(harga_paket) jumlah, '0' diskon from paket where id_paket in (select id_paket from tb_acara_workshop where id_siswa='$id_siswa')
union
select sum(harga) jumlah, '0' diskon from tb_pesanan_workshop where id_siswa='$id_siswa'
union
select '0' jumlah, diskon from tb_siswa_diskon where id_siswa='$id_siswa'
) a";
$rt=mysql_query($sql);
...
$sqlbayar="select * from tb_siswa_bayar where id_siswa=$id_siswa and id_bayar=$id_bayar";
$rst=@mysql_query($sqlbayar);
$tanggal=mysql_result($rst,0,"tanggal");
//echo $sqlbayar;
?>
		
<form name="form" method="post" action="<?="?menu=$menu&uid=$uid&page=$page&act=proses&id_siswa=$id_siswa&id_bayar=$id_bayar";?>">
<table>
...
<tr><td colspan=2 align=center><input type="submit" value="Delete" name="delete"> <input type="submit" value="Edit" name="edit"></td></tr>
<tr><td colspan=2 align=center><a href="<?="?menu=$menu&uid=$uid&page=siswa_nota&id_siswa=$id_siswa";?>"> 
...
$sql="delete from tb_siswa_bayar where id_siswa=$id_siswa and id_bayar=$id_bayar ";

$sql_hapus_bayar="insert into hapus_bayar_workshop values('','$id_siswa','$id_bayar','$tgl_bayar','$nilai','$keterangan','$pembayar','$catatan','$SAH[id_user]','$REMOTE_ADDR',now())";
...
where id_siswa=$id_siswa and id_bayar=$id_bayar ";
?>
<br><br>
<table cellpadding=3 cellspacing=0 style="border-width: 4px;  border-style: double;">
...
<? $id_siswa=mysql_result($rk,$k,"id_siswa");
	...
	echo "<a href=\"javascript:void(window.open('cetak_bukti_workshop.php?id_siswa=$id_siswa&id_bayar=$id_bayar','',''))\">"; ?><img border=0 src='images/Printer.png'></a></td>
<td align=center>
<? $id_siswa=mysql_result($rk,$k,"id_siswa");
	$id_bayar=mysql_result($rk,$k,"id_bayar");
	echo "<a href=?menu=$menu&uid=$uid&page=siswa_nota_edit&id_siswa=$id_siswa&id_bayar=$id_bayar>"?><img border=0 src='images/edit.gif'></a></td>

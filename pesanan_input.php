...
if (($SAH[id_group]==3) or ($SAH[id_group]==1) or ($SAH[id_group]==7)or ($SAH[id_group]==8) or ($SAH[id_group]==12))
{
...
$total12=$harga_bebas1*$jml_layanan1;
    ...
    if ($harga_bebas<0 and $SAH[id_group] > 1 and $SAH[id_group] < 12) 
    {
...
and b.id_plus='$id_plus'");

$m=@mysql_result($n,0,"jml");//echo $m;

$sql_bebas1="delete from client_busana where id_client='$id_client' and id_acara='$id_acara' 
and id_plus='$m'";
...
$sql_hapus_extra="insert into hapus_extra values('','$id_client','$id_acara','$id_plus','$m','$SAH[id_user]',now(),'$REMOTE_ADDR')";
mysql_query($sql_hapus_extra);
...
if($act=="del_paket"){
    ...
    mysql_query($sql_bebas1);//echo $sql_bebas1;
            ...
            $sql_hapus_paket="insert into hapus_paket values('','$id_client','$id_acara','$id_paket','$SAH[id_user]',now(),'$REMOTE_ADDR')";
            mysql_query($sql_hapus_paket);
    
	$act=null;
}

if($act=="del_acara"){
	$sql1="delete from acara where id_client='$id_client' and id_acara='$id_acara'";
	mysql_query($sql1);
	...
	mysql_query($sql3);
        ...
        $sql_hapus_acara="insert into hapus_acara values('','$id_client','$id_acara','$SAH[id_user]',now(),'$REMOTE_ADDR')";
        mysql_query($sql_hapus_acara);
...
<tr>
	...
	<a href="<?="?menu=$menu&uid=$uid&page=$page&act=update_acara&id_client=$id_client"?>"><img src='images/edit.gif' height='16' width='16' border=0></a>
	 <?php 
    } else { echo "<font color='#FF0000'><b>Hanya Group Administrator dan CS</b></font>";}
    ?>
...
echo "<option value=\"$p[id_layanan]\">$p[layanan]</option>\n";
    ...
    } else if (($SAH[id_group]==7) or ($SAH[id_group]==8)) {
    $propinsi = mysql_query("SELECT * FROM p_layanan where layanan='Tata Busana Fitting' ORDER BY layanan");
    while($p=mysql_fetch_array($propinsi)){
    echo "<option value=\"$p[id_layanan]\">$p[layanan]</option>\n";
    }
...
<tr><td>Jumlah Layanan </td><td>:&nbsp<input type="text" name="jml_layanan" required></td></tr>
...
if (($SAH[id_group]==3) or ($SAH[id_group]==1) or ($SAH[id_group]==12))
    {  
     echo "<a href=\"?menu=$menu&uid=$uid&page=$page&act=add_noextra&id_client=$id_client&idc=$idc\">[Tidak Ada Pilihan Layanan]</a>";
    } else { echo""; } ?>
</td></tr>
</table>
	</form>
...
<table width="564" height="105" border="0">
  ...
  <tr>
	...
	<?php
                    ...
                    echo "<div align='center'><font color='#FF0000'><b>Akses Tidak Diperbolehkan. Hanya Group Administrator dan CS yang bisa menambah Paket</b></font></div>";    
                    }                         
...
<?php
		...
		$selectacara = "<select size=1 name='$vrb_acara' class='edyellow combobox' required> $selectacara </select>";
		...
		$selectgaya = "<select size=1 name='$vrb_gaya' class='edyellow combobox' required> $selectgaya </select>";
		...
		$selectgaya1 = "<select size=1 name='$vrb_model' class='edyellow combobox' required> $selectgaya1 </select>";
		...
		$selectpaket = "<select size=1 name='$vrb_paket' class='edyellow combobox' required> $selectpaket </select>";
...
<?php
                    ...
                    echo "<div align='center'><font color='#FF0000'><b>Akses Tidak Diperbolehkan. Hanya Group Administrator dan CS yang bisa menambah acara</b></font></div>";    
                    }                        
}

...
$n=mysql_query("select id_tipe_baju, id_baju, jml_baju, id_tipe_kain, id_kain, jml_kain,id_tipe_acc,id_acc,jml_acc, id_tipe_sepatu, id_sepatu, jml_sepatu, id_blangkon, id_tipe_blangkon, jml_blangkon,id_tipe_celana,id_celana,jml_celana, id_client,id_acara,id_paket,id_paket 
...
{ 
	if ($jml == 0){
			...
			id_tipe_sepatu,id_sepatu,jml_sepatu,id_tipe_celana,id_celana,jml_celana
			)
			values(
			'$id_client','$id_acara','$id_paket','$id_plus',
			'$id_tipe_baju','$id_baju','$jml_baju',
			...
			'$id_tipe_celana','$id_celana','$jml_celana'						
			)";
			//echo $runSQL;
...
id_tipe_celana='$id_tipe_celana',id_celana='$id_celana',jml_celana='$jml_celana'
...
if ($run == ""){ 
	if ($jml != 0)
	{
		if($id_tipe_baju!="" and $id_tipe_acc!="" and $id_tipe_kain!="" and $id_tipe_sepatu!="" and $id_tipe_blangkon!=""  and $id_tipe_celana!="")
		{
		...
		else
		{
			...
			$id_tipe_celana=mysql_result($n,0,"id_tipe_celana");
			...
			$id_celana=mysql_result($n,0,"id_celana");
			...
			$jml_celana=mysql_result($n,0,"jml_celana")
			;
		}
...
";
        ...
        <tr>
          <td width="25%" align="left">Update Celana</td>
            <td width="25%" align="left">
            <select name="id_tipe_celana" onchange="javascript:this.form.submit();">
			<?
				$sql="select id_tipe_baju id_tipe_celana,keterangan from p_baju_tipe where upper(keterangan) like '%CELANA%' 
UNION ALL
select id_tipe_baju id_tipe_celana,keterangan from p_baju_tipe where upper(keterangan) like '%ROK%' 
";
				$rs=mysql_query($sql);
				for($a=0;$a<mysql_num_rows($rs);$a++){
				echo "<option ";
				$idp=mysql_result($rs,$a,"id_tipe_celana");
				if ($idp==$id_tipe_celana)echo " selected ";
				echo " value=\"$idp\">";
				echo mysql_result($rs,$a,"keterangan");
				echo "</option>";
				} 
			...
			<td width="25%" align="left"><select name="id_celana" value="<?=$id_celana;?>">
			<?
				if(!isset($id_tipe_celana))$id_tipe_celana=1;
				$sql="select id_layanan,layanan from p_baju where id_tipe_baju='$id_tipe_celana' order by layanan asc";
				$rs=mysql_query($sql);
				for($a=0;$a<mysql_num_rows($rs);$a++){
				echo "<option ";
				$idl=mysql_result($rs,$a,"id_layanan");
				if ($idl==$id_celana)echo " selected ";
				echo " value=\"$idl\">";
				echo mysql_result($rs,$a,"layanan");
				echo "</option>";
				} 
            ...
            <td width="25%" align="center"><input type="text" name="jml_celana" value="<?=$jml_celana;?>" size="5"> 
            <font color="#FF0000"><b>*</b></font></td>
        </tr>
		</table>
...
//}
//else
//{ echo "<div align='center'><font color='#FF0000'><b>Akses Tidak Diperbolehkan. Hanya Group Administrator dan Fitting</b></font></div>"; }

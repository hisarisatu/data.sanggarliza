...
//if (strlen($id_pegawai) < 1){ $iid_pegawai = "<br><font color='#FF0000' size='1'><i>* Petugas CS tidak valid"; $ok=0; }
	...
	if (($ok == 1) and ($id == "")){
		...
		$runSQL = "insert into non_client (nama, divisi, detail, jumlah, tanggal, jenis_bayar, bank, created, kwitansi) VALUES ('$nama', '$id_jenis_bagian', '$detail', '$jumlah', '$new_tanggal', '$id_jenis_bayar', '$id_bank', now(),'$newID')";
		//echo $runSQL;
		$insert = mysql_query($runSQL, $connDB);
		$id = mysql_insert_id($connDB);
	} else if (($ok == 1) and ($id <> "")){
		$registerInvalid = 1;
		$runSQL = "update non_client set nama='$nama', divisi='$id_jenis_bagian', detail='$detail', jumlah='$jumlah', tanggal='$new_tanggal', jenis_bayar='$id_jenis_bayar', bank='$id_bank' where id_non_client='$id'";
		//echo $runSQL;

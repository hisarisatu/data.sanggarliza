...
<tr><td>Ukuran Beskap </td><td>:	<?
			...
			union select 0,'--Pilih Ukuran Beskap--' from dual";
			generate_select("ukuran_beskap",$sqlbeskap,$ukuran_beskap);
			?></td></tr>
<tr><td>Ukuran Blangkon </td><td>:	<?
			$sqlblangkon="select 55,'55' from dual 
			union select 56,'56' from dual
			union select 57,'57' from dual
			union select 58,'58' from dual
			union select 59,'59' from dual
			union select 50,'-Pilih Ukuran Blangkon-' from dual";
			generate_select("ukuran_blangkon",$sqlblangkon,$ukuran_blangkon);
			?></td></tr>
<tr><td>Ukuran Selop </td><td>:	<?
			$sqlselop="select 39,'39' from dual 
			union select 40,'40' from dual
			union select 41,'41' from dual
			union select 42,'42' from dual
			union select 43,'43' from dual
			union select 44,'44' from dual
			union select 0,'--Pilih Ukuran Selop---' from dual";
...
pakai_kebaya=$pakai_kebaya,pakai_acc=pakai_acc
where id_ukur=$id_ukur";
mysql_query($sqlupd);
//echo "<br>$sqlupd";
$act=null;
...
(id_client,id_acara,id_paket,id_sublayanan,nama,id_jabatan,asal_keluarga,id_sanggul,jenis_rias,pakai_kain,pakai_kebaya,pakai_acc)
values ($id_client,$id_acara,$id_paket,$id_sublayanan,'$nama',$id_jabatan,$asal_pihak,$id_sanggul,$jenis_rias,$pakai_kain,$pakai_kebaya,$pakai_acc)";
} else 
...
echo "<table width='100%' cellspacing='1' cellpadding='3'>";
echo "<tr bgcolor='#A7A7A7' height='25'>";
echo "<td><b>No</td><td><b>Nama</td><td>Jabatan</td><td>Asal Keluarga</td><td>Model Sanggul</td><td>Jenis Rias</td><td>Pakai Kain</td><td>Pakai Kebaya</td><td>Pakai Accesoris</td><td>Update</td></tr>";
for ($k=0;$k<@mysql_num_rows($rk);$k++){
$ccc++;
    if ($ccc%2 > 0){ $color="#EBEFFA"; }else{ $color="#D7E0F4"; };
    echo "

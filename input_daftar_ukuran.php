...
<tr><td>Ukuran Beskap </td><td>:	<?
			$sqlbeskap=" select 55,'--Pilih Ukuran Beskap--' from dual
			union select 1,'L1' from dual 
			...
			union select 13,'XLL' from dual
                        union select 82,'PENDING' from dual   
                        union select 56,'XXXLL' from dual
                        union select 57,'XXXXLL' from dual
                        union select 58,'15' from dual
                        union select 59,'15.5' from dual
                        union select 60,'16' from dual
                        union select 61,'16.5' from dual
                        union select 62,'17' from dual
                        union select 63,'17.5' from dual
                        union select 64,'18' from dual
                        union select 70,'A' from dual
			union select 71,'B' from dual
			union select 72,'C' from dual
			union select 73,'D' from dual
                        union select 74,'E' from dual
			union select 75,'F' from dual
			union select 76,'G' from dual
			union select 77,'H' from dual
			union select 78,'PERDANA' from dual
			union select 79,'HAK MILIK' from dual
			union select 80,'SENDIRI' from dual";
			generate_select("ukuran_beskap",$sqlbeskap,$ukuran_beskap);
			?></td></tr>
<tr><td>Ukuran Blangkon </td><td>:	<?
			$sqlblangkon=" select 55,'-Pilih Ukuran Blangkon-' from dual
			union select 82,'PENDING' from dual 
                        union select 65,'50' from dual 
                        union select 66,'51' from dual
			union select 67,'52' from dual
			union select 68,'53' from dual
			union select 69,'54' from dual
                        union select 14,'55' from dual
...
<tr><td>Ukuran Selop </td><td>:	<?
			$sqlselop=" select 55,'--Pilih Ukuran Selop---' from dual
			union select 82,'PENDING' from dual 
                        union select 32,'26' from dual 
			...
			?></td></tr>	
<tr><td>Ukuran Celana </td><td>:	<?
			$sqlcelana="select 55,'--Pilih Ukuran Celana---' from dual
			union select 82,'PENDING' from dual 
                        union select 1,'L1' from dual 
			union select 2,'L2' from dual
			union select 3,'L3' from dual
			...
			union select 80,'SENDIRI' from dual
		";
			generate_select("ukuran_celana",$sqlcelana,$ukuran_celana);
			?></td></tr>						
<? }
...
<tr><td>Status Rias </td><td>:	<?
			...
			union select 3,'Tidak Rias' from dual
                        union select 4, 'Retouch' from dual
...
nama='$nama',id_jabatan=$id_jabatan,asal_keluarga=$asal_pihak,ukuran_beskap=$ukuran_beskap,ukuran_blangkon=$ukuran_blangkon,ukuran_selop=$ukuran_selop,ukuran_celana=$ukuran_celana
...
(id_client,id_acara,id_paket,id_sublayanan,nama,id_jabatan,asal_keluarga,ukuran_beskap,ukuran_blangkon,ukuran_selop,ukuran_celana)
values ($id_client,$id_acara,$id_paket,$id_sublayanan,'$nama',$id_jabatan,$asal_pihak,$ukuran_beskap,$ukuran_blangkon,$ukuran_selop,$ukuran_celana)";
} else 
...
case
when ukuran_beskap=82 then 'PENDING' 
...
when ukuran_beskap=56 then 'XXXL'
when ukuran_beskap=57 then 'XXXXL'
when ukuran_beskap=58 then '15'
when ukuran_beskap=59 then '15.5'
when ukuran_beskap=60 then '16'
when ukuran_beskap=61 then '16.5'
when ukuran_beskap=62 then '17'
when ukuran_beskap=63 then '17.5'
when ukuran_beskap=64 then '18'
when ukuran_beskap=70 then 'A'
when ukuran_beskap=71 then 'B'
when ukuran_beskap=72 then 'C'
when ukuran_beskap=73 then 'D'
when ukuran_beskap=74 then 'E'
when ukuran_beskap=75 then 'F'
when ukuran_beskap=76 then 'G'
when ukuran_beskap=76 then 'H'
when ukuran_beskap =78 then 'PERDANA'
when ukuran_beskap =79 then 'HAK MILIK'
when ukuran_beskap =80 then 'SENDIRI'
when ukuran_beskap=55 then '-'
end as ukuran_beskap,ukuran_beskap as ukuran_beskap1,
case
when ukuran_blangkon = 82 then 'PENDING'
when ukuran_blangkon = 65 then 50
when ukuran_blangkon = 66 then 51
when ukuran_blangkon = 67 then 52
when ukuran_blangkon = 68 then 53
when ukuran_blangkon = 69 then 54
...
when ukuran_blangkon =78 then 'PERDANA'
when ukuran_blangkon =79 then 'HAK MILIK'
when ukuran_blangkon =80 then 'SENDIRI'
when ukuran_blangkon = 55 then '-'
end as ukuran_blangkon
,
case
when ukuran_selop =82 then 'PENDING'
...
when ukuran_selop =78 then 'PERDANA'
when ukuran_selop =79 then 'HAK MILIK'
when ukuran_selop =80 then 'SENDIRI'

when ukuran_selop =55 then '-'
end as ukuran_selop
,
case
when ukuran_celana=82 then 'PENDING'
when ukuran_celana=1 then 'L1'
when ukuran_celana=2 then 'L2'
when ukuran_celana=3 then 'L3'
when ukuran_celana=4 then 'L4'
when ukuran_celana=5 then 'L5'
when ukuran_celana=6 then 'L6'
when ukuran_celana=7 then 'L7'
when ukuran_celana=8 then 'L8'
when ukuran_celana=25 then 'L9'
when ukuran_celana=9 then 'S'
when ukuran_celana=10 then 'M'
when ukuran_celana=11 then 'L'
when ukuran_celana=12 then 'XL'
when ukuran_celana=13 then 'XXL'
when ukuran_celana=56 then 'XXXL'
when ukuran_celana=57 then 'XXXXL'
when ukuran_celana =78 then 'PERDANA'
when ukuran_celana =79 then 'HAK MILIK'
when ukuran_celana =80 then 'SENDIRI'
when ukuran_celana =55 then '-'
end as ukuran_celana
...
echo "<td><b>No</td><td><b>Nama</td><td>Jabatan</td><td>Asal Keluarga</td><td>Ukuran Beskap</td><td>Ukuran Blangkon</td><td>Ukuran Selop</td><td>Ukuran Celana
</td><td>Update</td></tr>";
...
<td><?=mysql_result($rk,$k,"ukuran_celana")?></td>
...
when jenis_rias=4 then 'Retouch'
...
case
when ukuran_kebaya=82 then 'PENDING' 

...
if (($SAH[id_group]==1)or($SAH[id_group]==7))
{
function js_submit()
{
...
<br>
<h2 align="center"> Form konsultasi FITTING </h3>
<hr>
<br>
...
<form method="post" action="<?="?menu=$menu&uid=$uid&page=client_konsultasi1&act=proses&id_client=$id_client";?>">
<table>
...
<tr><td>Petugas :</td><td>	<?
			...
			where a.id_pegawai=b.id_pegawai and b.id_pekerjaan=27 union select 0,'--Pilih Petugas Fitting--' from dual";
...
$sql="insert into konsultasi_fiting
(id_client,id_konsultasi1,tanggal,isi,id_user,login_ip,created,Client_hadir,Petugas)
...
$sql = "select tanggal ,id_konsultasi1, isi,
client_hadir,petugas,nama,id_client
from konsultasi_fiting a,pegawai b
WHERE a.petugas = b.id_pegawai 
AND id_client='$id_client'
and id_konsultasi1 like '$id_konsul'
...
<td><a href='?menu=$menu&uid=<?=$uid?>&page=client_konsultasi1&id_client=<?=mysql_result($rk,$k,"id_client")?>
&id_konsul=<?=mysql_result($rk,$k,"id_konsultasi1")?>
...
}else{ echo "<div align='center'><font color='#FF0000'><b>Akses Tidak Diperbolehkan. Hanya Group Administrator dan Fitting</b></font></div>"; }

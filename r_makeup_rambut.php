...
where 1=1 $filterSQL1
group by id_client, id_acara, tanggal,acara, tempat, nama_cpw, nama_cpp, layanan, id_layanan) b
order by  nama asc, tanggal asc, waktu asc
        ...
        <fieldset style="width:400"><legend><strong>Search</strong></legend>
            <table>
                ...
                <tr>
                    ...
                    <td>    <?php
                        js_submit();
                        $sql="select distinct id_layanan,layanan from p_layanan union select 0,'--All Layanan--' from dual";
...
<script type="text/javascript">//<![CDATA[
    ...
    //]]></script>-->
<div align="center"><font class="titledata"><b>Laporan Penjadwalan Total Jumlah Tata Rias Makeup dan Rambut</b></font></div>
...
$sq=mysql_query("select a.id_client, a.id_acara, sum(jml_orang)
from pesanan_plus a, p_sublayanan b
where a.id_sublayanan=b.id_sublayanan
and b.id_layanan=1
group by b.id_layanan, '$id_client', '$id_acara'");
$jml_m=mysql_result($sq,0,"sum(jml_orang)");


  $se=mysql_query("select a.id_client, a.id_acara, sum(jml_orang)
from pesanan_plus a, p_sublayanan b
where a.id_sublayanan=b.id_sublayanan
and b.id_layanan=18
group by b.id_layanan, '$id_client', '$id_acara' ");
  $jml_r=mysql_result($se,0,"sum(jml_orang)");
  
  $st=mysql_query("select a.id_gaya, b.gaya from acara a, p_gaya b where a.id_acara='$id_acara' and a.id_client='$id_client' and  a.id_gaya=b.id_gaya 
group by id_client, id_acara");
 
  $su=mysql_query("select a.id_sanggul, b.model_sanggul from acara a, p_model_sanggul b where a.id_acara='$id_acara' and a.id_client='$id_client' and a.id_sanggul=b.id_sanggul group by id_client, id_acara ");
  
  /*$fe=mysql_query("select a.id_client, a.id_acara, a.id_paket, sum(jml_orang)
from acara a, paket_sub_paket b, p_sublayanan c
where a.id_paket=b.id_paket and b.id_sublayanan=c.id_sublayanan
and c.id_layanan=1
group by c.id_layanan, '$id_client', '$id_acara'");
  $jml_pm=mysql_result($fe,0,"sum(jml_orang)");
  
    $fo=mysql_query("select a.id_client, a.id_acara, a.id_paket, sum(jml_orang)
from acara a, paket_sub_paket b, p_sublayanan c
where a.id_paket=b.id_paket and b.id_sublayanan=c.id_sublayanan
and c.id_layanan=18
group by c.id_layanan, '$id_client', '$id_acara'");
	$jml_pr=mysql_result($fo,0,"sum(jml_orang)");
	
	$total_m=($jml_m+$jml_pm);
	$total_r=($jml_r+$jml_pr);
  ...
  */
    ...
    <td>
        <?php
        while($gaya=mysql_fetch_array($st))
        {
        echo $gaya['gaya'];
        echo "<br>";
        }
    ...
    <td>
        <?php
        while($model_sanggul=mysql_fetch_array($su))
        {
  ...
  <td>
       <?php
        while($makeup=mysql_fetch_array($sq))
        {
        echo $makeup['sum(jml_orang)'];
        echo "<br>";
        }
        ?>
    ...
    <td>
       <?php
        while($rambut=mysql_fetch_array($se))
        {
        echo  $rambut['sum(jml_orang)'];
        echo "<br>";
        }
        ?>
...
while($rec=mysql_fetch_array($res))
{
 ...
 // $sql_gaya="select distinct b.gaya,a.id_gaya from acara a inner join p_gaya b on a.id_gaya=b.id_gaya and id_client=".$rec['id_client']." and id_acara=".$rec['id_acara']."";

$sql_makeup="select sum(jml_orang) from pesanan_plus a  inner join p_sublayanan b on a.id_sublayanan=b.id_sublayanan where b.id_layanan=1 and a.id_client=".$rec['id_client']." and a.id_acara=".$rec['id_acara']." group by b.id_layanan";

//$data_makeup=  mysql_query($sql_makeup);
//$jml_m1=@mysql_result($data_makeup,0,"sum(jml_orang)");


 $data_makeup=  mysql_query($sql_makeup);
 $data_makeup= mysql_fetch_assoc($data_makeup);
  
    //$sql_sanggul="select distinct b.model_sanggul,a.id_sanggul from acara a inner join p_model_sanggul b on a.id_sanggul=b.id_sanggul and id_client=".$rec['id_client']." and id_acara=".$rec['id_acara']."";

$sql_rambut="select sum(jml_orang) from pesanan_plus a inner join p_sublayanan b on a.id_sublayanan=b.id_sublayanan where b.id_layanan=18 and a.id_client=".$rec['id_client']." and a.id_acara=".$rec['id_acara']." group by b.id_layanan";
 
 //$data_rambut=  mysql_query($sql_rambut);
//$jml_r1=@mysql_result($data_rambut,0,"sum(jml_orang)");


 $data_rambut=  mysql_query($sql_rambut);
 $data_rambut= mysql_fetch_assoc($data_rambut);
  
/*  
   $paket_m="select a.id_client, a.id_acara, a.id_paket, sum(jml_orang)
from acara a, paket_sub_paket b, p_sublayanan c
where a.id_paket=b.id_paket and b.id_sublayanan=c.id_sublayanan
and c.id_layanan=1
group by c.id_layanan, '$id_client', '$id_acara'";
 $data_paketm=  mysql_query($paket_m);
  $jml_pm1=mysql_result($data_paketm,0,"sum(jml_orang)");
  
    $paket_r="select a.id_client, a.id_acara, a.id_paket, sum(jml_orang)
from acara a, paket_sub_paket b, p_sublayanan c
where a.id_paket=b.id_paket and b.id_sublayanan=c.id_sublayanan
and c.id_layanan=18
group by c.id_layanan, '$id_client', '$id_acara'";
 $data_paketr=  mysql_query($paket_r);
	$jml_pr1=mysql_result($data_paketr,0,"sum(jml_orang)");
	
	$total_m1=($jml_m1+$jml_pm1);
	$total_r1=($jml_r1+$jml_pr1);
  echo $total_m1;
  echo $jml_m1;
  echo $jml_pm1;
  echo"</br>";
...
<td><?= $rec['nama']; ?></td>
<td><?= $data_cs['nama']; ?></td>
<td align="center"><?= $data_makeup['sum(jml_orang)']; ?></td>
<td align="center"><?= $data_rambut['sum(jml_orang)'];?></td>
</td>
</tr>
<?php 
//echo $runSQL;

...
$runSQL = "select count(*) total from (Select a.id_non_client,a.nama, ifnull(b.keterangan,'-')divisi, a.detail, a.tanggal,  a.jumlah, a.catatan
from non_client a
...
unset($ii);
$runSQL = "Select a.id_non_client,a.nama, ifnull(b.keterangan,'-')divisi, a.detail, DATE_FORMAT(a.tanggal, '%Y-%m-%d')tanggal, a.jumlah, a.kwitansi, a.catatan
from non_client a
left join p_pengeluaran b on a.divisi=b.id_jenis_bagian
...
$htmlData .= "<td align='center' nowrap>
		   <a href=\"javascript:void(window.open('cetak_bukti_non.php?id_non_client=$row[id_non_client]&nama=$row[nama]&kwitansi=$row[kwitansi]&divisi=$row[divisi]&tanggal=$row[tanggal]&detail=$row[detail]&jumlah=$row[jumlah]&catatan=$row[catatan]&id_user=$SAH[id_user]&login_ip=$REMOTE_ADDR&created=$sekarang'));\"><img border='0' src='images/Printer.png'></a>";
     ...
     <input type='text' name='tanggal' id="tanggal" size='11' value='<?=$tanggal?>'>
		...
		<input type='text' name='tanggal2' id="tanggal2" size='11' value='<?=$tanggal2?>'>
           ...
           <td valign="bottom" align="right">
			 ...
			 <b><a href="<?="?menu=$menu&uid=$uid&page=non_client_input";?>">Input Baru</a> <!--| <a href="<?//= "?menu=$menu&uid=$uid&page=client&act=cari";?>">Cari</a>--></b>
		 </td>
       
     <td valign="bottom" align="right">
			 <img src="images/arrow2.gif" border="0">
			 <b><a href="<?="?menu=$menu&uid=$uid&page=non_client";?>">List All</a> <!--| <a href="<?//= "?menu=$menu&uid=$uid&page=client&act=cari";?>">Cari</a>--></b>
		 </td>
    ...
    //]]></script>-->
	 ...
	 <table width='99%' cellspacing='1' cellpadding='3'>
		...
		<tr bgcolor='#A7A7A7' height="25">
            ...
            <td width="6%" align='center'>TANGGAL</td>
			<td width="14%" align='center'>DETAIL</td>
			<td width="7%" align='center'>JUMLAH</td>
			<td width="8%" align='center'>CATATAN</td>

...
<a href=""><img border=0 src='images/Printer.png' title='Cetak Rincian'></a>&nbsp;&nbsp;
...
?>	 
	 ...
	 <table width="100%" border="0" cellpadding="0" cellspacing="0">
	  <tr>
     <td width="50%" valign="bottom" align="left"> &nbsp; 
	     <font class="titledata" color="#009900"><b>Rincian Penggunaan Sumber Daya</b></font>
		 </td>
     <td width="50%" valign="bottom" align="right">
...
for($n1=0;$n1<mysql_num_rows($res);$n1++){
 ...
 for($p=0;$p<@mysql_num_rows($rp);$p++){
	...
	$np=mysql_query("select a.nama_paket,a.harga_paket,sum(b.jml_orang) jumlah from paket a,paket_sub_paket b where a.id_paket='$id_paket' and a.id_paket=b.id_paket group by a.nama_paket, a.harga_paket");
	echo "<tr><td>";
	if(mysql_num_rows($np)!=0){
		echo mysql_result($np,0,"nama_paket");
		echo "</td><td align=right>";
		echo number_format(mysql_result($np,0,"jumlah"),0);
		echo "</td></tr><tr><td nowrap>";
		$sql="select a.detail_layanan,b.jml_orang,b.satuan from p_sublayanan a,paket_sub_paket b where a.id_sublayanan=b.id_sublayanan and b.id_paket='$id_paket'";
		$rs=mysql_query($sql);
		$n=mysql_num_rows($rs);
		for($a=0;$a<$n;$a++){
		echo "&nbsp;&nbsp;&raquo;&nbsp;";
		...
		echo "</td><td align=right>";
        echo number_format(mysql_result($rs,$a,"jml_orang"),0);
echo " ";
echo mysql_result($rs,$a,"satuan");
        echo "</td></tr><tr><td nowrap>";
	 }
	}
}

$sql="select b.detail_layanan,a.jml_orang,a.harga,a.satuan from pesanan_plus a, p_sublayanan b where a.id_sublayanan=b.id_sublayanan and a.id_client='$id_client' and id_acara='$id_acara'"; //echo $sql;
$rpp=mysql_query($sql);
...
for($p=0;$p<$jpp;$p++){
 ...
 //echo number_format(mysql_result($rpp,$p,"jml_orang"),0);
 echo "</td><td align=right>";
 echo number_format(mysql_result($rpp,$p,"jml_orang"),0);
 echo " ";
 echo mysql_result($rpp,$p,"satuan");
 echo "</td></tr>";
}
/*
echo "<tr><td colspan=2><a href='sub_pesanan_input.php?id_client=$id&id_acara=$id_acara&id_paket=$id_paket&menu=$menu&uid=$uid&page=$page&id=$id' target='iframe_pesanan'><img border='0' src='images/add.gif' title='Tambahan Layanan' width=15 height=15> Tambah Layanan</a></td></tr>";
*/
...
from 
(
select sum(b.jml_orang) jumlah from acara a, paket_sub_paket b where a.id_client='$id_client' and a.id_paket=b.id_paket
union
select sum(jml_orang) jumlah from pesanan_plus where id_client='$id_client'
) a";//echo $sql;
$rt=mysql_query($sql);
$total=@mysql_result($rt,0,"total");
?>
<tr>
<td><b>Total Penggunaan Sumber Daya</b></td>
<td align=right style="border-width: 1px; border-style: double;"><font style="font-size:larger;"><u><?=number_format($total,0)?></td>
</tr>
<tr>
<td align=right colspan=2><b><?=katakan($total)?></td>
</tr>
</table>
...
<!--
<tr><td>
<iframe width="750" frameborder=0 name="iframe_pesanan"></iframe>
</td></tr>
-->
</table>
		 </td>
